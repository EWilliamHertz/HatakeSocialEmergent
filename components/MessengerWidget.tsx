...
export default function MessengerWidget() {
  ...
  const uploadMedia = async (file: File) => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        // Once the image is uploaded, send it as a message
                        ...
                        return (
                          <div key={msg.message_id}>
                            ...
                            <div className={`flex gap-2 group/msg ${msg.sender_id === currentUserId ? 'flex-row-reverse' : ''}`}>
                              ...
                              {/* Avatar */}
                              {msg.picture ? (
                                <Image src={msg.picture} alt={msg.name} width={28} height={28} className="rounded-full flex-shrink-0 w-7 h-7 object-cover" unoptimized />
                              ) : (
                    ...
                    <div className="border-t dark:border-gray-700 p-2">
                      ...
                      <div className="flex gap-2">
                        <textarea
                          ...
                          placeholder={uploading ? "Uploading..." : "Type... (Ctrl+V to paste image)"}
                          disabled={uploading}
                          rows={1}
                          className="flex-1 px-3 py-2 text-sm border rounded-lg focus:ring-2 focus:ring-blue-500 resize-none dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                  ...
                  <div ref={messagesContainerRef} className="flex-1 overflow-y-auto p-3 space-y-2" onScroll={handleMessagesScroll}>
                    {groupMessages.map((msg) => (
                      <div key={msg.message_id} className={`flex gap-2 ${msg.sender_id === currentUserId ? 'flex-row-reverse' : ''}`}>
                        {msg.picture
                          ? <Image src={msg.picture} alt={msg.name} width={24} height={24} className="rounded-full flex-shrink-0 object-cover" unoptimized />
                  ...
                  <div className="flex-1 overflow-y-auto">
                    {groupChats.length === 0 ? (
                      ...
                      groupChats.map(group => (
                        ...
                        >
                          {group.image
                            ? <Image src={group.image} alt={group.name} width={36} height={36} className="rounded-xl object-cover" unoptimized />
