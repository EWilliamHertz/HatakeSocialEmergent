...
import { Package, BookOpen, LayoutGrid, List as ListIcon, Star, Loader2, Search, Heart, X } from 'lucide-react';
...
interface BookmarkedCollection {
  user_id: string;
  username: string;
...
export default function CollectionPage() {
  ...
  const [activeTab, setActiveTab] = useState<'cards' | 'sealed' | 'wishlist' | 'bookmarks'>('cards');
  const [items, setItems] = useState<CollectionItem[]>([]);
  const [bookmarkedCollections, setBookmarkedCollections] = useState<BookmarkedCollection[]>([]);
  ...
  const removeBookmark = async (userId: string) => {
    try {
      ...
      if (res.ok) {
        setBookmarkedCollections(prev => prev.filter(c => c.user_id !== userId));
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 mb-6">
          <h1 className="text-3xl font-bold dark:text-white mb-6">My Collection</h1>
          ...
          <div className="flex gap-2 border-b border-gray-200 dark:border-gray-700 mb-6 overflow-x-auto">
            ...
            <button
              ...
              className={`px-4 py-3 font-semibold whitespace-nowrap transition border-b-2 flex items-center gap-2 ${
                activeTab === 'bookmarks'
                  ? 'border-blue-600 text-blue-600 dark:text-blue-400'
                  : 'border-transparent text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200'
              }`}
          ...
          {activeTab === 'sealed' && (
            <div className="text-center py-8">
              ...
              <p className="text-gray-500 dark:text-gray-400">Coming soon - Track sealed products like booster boxes and sets</p>
            </div>
          )}
          ...
          {activeTab === 'wishlist' && (
            <div className="text-center py-8">
              ...
              <p className="text-gray-500 dark:text-gray-400">Coming soon - Create a wishlist of cards you want</p>
            </div>
          )}
        </div>
          ...
          <div>
            {bookmarkedCollections.length === 0 ? (
              <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-12 text-center">
                ...
                <p className="text-gray-500 dark:text-gray-400">No bookmarked collections yet. Star collections to follow collectors!</p>
              </div>
            ) : (
                    ...
                    <div className="p-6">
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <h3 className="font-semibold dark:text-white">{collection.username}</h3>
                            <p className="text-sm text-gray-500 dark:text-gray-400">{collection.card_count} cards</p>
                          </div>
                        </div>
                        <button
                          onClick={() => removeBookmark(collection.user_id)}
                      ...
                      <div className="mb-4 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                        <p className="text-xs text-gray-500 dark:text-gray-400">Collection Value</p>
                        <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">€{collection.total_value.toFixed(2)}</p>
                      </div>

                      <Link
                        href={`/collection/user/${collection.user_id}`}
